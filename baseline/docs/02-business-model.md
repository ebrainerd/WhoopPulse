...
- The **live media kit** with API-verified stats, one published kit,
  "Made with Baseline" footer branding, standard light theme.
...
- **Explain**: baselines and z-score badges, first-24h velocity tracking with
  breakout push alerts, weekly LLM-narrated report with "show the math".
- **Prove**: kit customization (themes, remove Baseline branding), rate card
...
**14 days of Pro, card-optional, starting at Instagram connect** (not at
...
- Day 12: email + push, plain copy, shows *their own numbers* ("Baseline has
...
Why card-optional instead of card-required: our positioning attacks
...
- App-store billing on mobile (users can also cancel via Apple/Google) even
  though it costs us 15–30% commission — the trust posture is worth the margin
  on mobile-originated subscriptions.
...
| Payments | ~$0.35–3.00 | Stripe ~2.9%+30¢ on web; Apple/Google 15% (small-business tier) on mobile |
| **Total COGS** | **≈ $0.55–3.50** | **Gross margin ≈ 65–95% depending on billing channel** |
...
| --- | --- | --- |
| Visitor → signup | 8% | marketing-site CTA conversion |
...
| Trial → paid | 12–18% (card-optional norm) | conversion at day 14–104 (grace window rescues late converters) |
...
1. **The kit loop (structural).** Free kits carry "Made with Baseline"; kits
...
4. **Brand-side surface (long-term option)** — brands viewing many Baseline
...
| **Baseline Pro** | **$10/mo** | full archive + explanations + media kit, one honest tier |
...
| App-store commission compresses margin | Steer annual plans to web checkout (allowed via external-purchase rules in most regions now); mobile monthly stays app-store for trust |
